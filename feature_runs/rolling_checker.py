...
if __name__ == '__main__':
    ...
    rolling_mask_even = np.zeros(ra.size, dtype=bool)
    rolling_mask_odd = np.zeros(ra.size, dtype=bool)

    # Well, I think this is a compact way to set the bins.
    n_bins = 4
    north_oss = np.sin(ra*n_bins)
    south_oss = -1.*north_oss

    rolling_mask_even[np.where((north_oss > 0) & (dec > dec_limit) & (wfd > 0))] = True
    rolling_mask_even[np.where((south_oss > 0) & (dec <= dec_limit) & (wfd > 0))] = True

    rolling_mask_odd[np.where((north_oss <= 0) & (dec > dec_limit) & (wfd > 0))] = True
    rolling_mask_odd[np.where((south_oss <= 0) & (dec <= dec_limit) & (wfd > 0))] = True
    ...
    for filtername in filters:
        ...
        bfs.append(fs.Rolling_mask_basis_function(rolling_mask_even, year_mod=2, year_offset=0,
                                                  mjd_start=observatory.mjd, nside=nside))
        bfs.append(fs.Rolling_mask_basis_function(rolling_mask_odd, year_mod=2, year_offset=1,
                                                  mjd_start=observatory.mjd, nside=nside))
    ...
    observatory, scheduler, observations = fs.sim_runner(observatory, scheduler,
                                                         ...
                                                         filename='feature_rolling_checker_%iyrs.db' % years,
                                                         delete_past=True)
